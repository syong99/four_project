...
export default {
  ...
  methods: {
    ...
    async getadeDetail() {
      let adeDetail = await this.$api('/api/adeDetail2', { param: [this.productId] })
    ...
    async getProductImage() {
      this.productImage = await this.$api('/api/adeimageList', { param: [this.productId] })
    ...
    deleteImage(id) {
      this.$swal.fire({
        ...
        if (result.isConfirmed) {
          await this.$api("/api/adeimageDelete", { param: [id] })
          this.getProductImage()
