   ...
   async getCategoryList(){
	...
	changeCategory2(){
	 ...
	 productUpdate() {
      ...
      if(this.product.created_date == "" || this.product.created_date == 0) {
        return this.$swal("출고일을 입력하세요.");
      }

    //   this.adeDetail.category_id = this.categoryList.filter(c => {
    //     return (c.category1 == this.cate1 && c.category2 == this.cate2 && c.category3 == this.cate3);
    //   })[0].id;

    //   console.log(this.adeDetail.category_id);
      ...
      this.$swal.fire({
        ...
        if (result.isConfirmed) {
          await this.$api("/api/teaInsert",{param:[this.product]});
